...
def load_all_response_values(cfg, model_key, models, metric, region, lead_start, lead_end):
    ...
    for variable, level in enabled_variable_levels(cfg):
        ...
        if model_key not in raw_by_model:
            continue

        pooled = np.concatenate(list(raw_by_model.values()))
        ...
        norm_vals = raw_by_model[model_key] / denom
...
def main():
    ...
    parser.add_argument("--plot-config", default="config/all_response_violin_no_hrrr.yaml")
